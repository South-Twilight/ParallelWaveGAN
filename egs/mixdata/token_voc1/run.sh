...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    for db_dir in "$db_root"/*/; do
        ...
        sort -o data/database/$dbname/wav.scp data/database/$dbname/wav.scp

        head -n $dev_num data/database/$dbname/wav.scp >> data/${dev_set}/wav.scp
        head -n $((dev_num + eval_num)) data/database/$dbname/wav.scp > data/${eval_set}/wav.scp.tmp
        tail -n $eval_num data/${eval_set}/wav.scp.tmp >> data/${eval_set}/wav.scp
        rm data/${eval_set}/wav.scp.tmp
        tail -n $train_num data/database/$dbname/wav.scp >> data/${train_set}/wav.scp
